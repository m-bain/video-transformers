"""
TimeSformer in PyTorch, with ViT initialisation
...
class SpaceTimeBlock(nn.Module):
    ...
    def forward(self, x, einops_from_space, einops_to_space, einops_from_time, einops_to_time,
                time_n, space_f):
        if self.attention_style == 'divided':
            ...
            space_output = self.attn(self.norm1(time_residual_norm), einops_from_space,
...
class Timesformer(nn.Module):
    ...
    Notable differences: allows for variable length input frames equal <= num_framese
    """

    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, representation_size=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., hybrid_backbone=None, norm_layer=None,
                 num_frames=8, time_init='rand', border_size=None, num_borders=0, attention_style='divided'):
        """
        Args:
...
def load_from_vit(timesformer_model, vit_model):
    vit_model.head = nn.Identity()
    vit_checkpoint = vit_model.state_dict()
    load_pos_embed = vit_checkpoint['pos_embed']
    curr_pos_embed = timesformer_model.state_dict()['pos_embed']
    ...
    timesformer_model.load_state_dict(vit_checkpoint, strict=False)

    return timesformer_model


def timesformer_base_patch16_224(pretrained=False, **kwargs):
    """Timesformer-base (Tformer-B/16).
    Pretrained is using ViT ImageNet-1k weights fine-tuneeed from in21k @ 224x224"""

    model_params = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = Timesformer(**model_params)

    if pretrained:
        vit_model = vision_transformer.vit_base_patch16_224(pretrained=True)
        model = load_from_vit(model, vit_model)

    return model


if __name__ == "__main__":
    timesformer = timesformer_base_patch16_224(pretrained=True, time_init='zeros', num_frames=4, num_classes=5)
    vit_model = vision_transformer.vit_base_patch16_224(pretrained=True)
    vit_model.head = nn.Identity()
    timesformer.head = nn.Identity()
    timesformer.pre_logits = nn.Identity()
    imgs = torch.rand([2, 4, 3, 224, 224])

    print('TIMESFORMER OUTPUT:')
    output = timesformer(imgs)
    print(output.shape)
    ...
    for idx in range(imgs.shape[1]):
        ...
        print(vit_output.min(), vit_output.max())
        break